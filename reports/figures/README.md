...
| 10 | `10_arc_submission_scorecard` | What did the first real validation submission score, member by member? | *Not generated: no real score exists yet.* Drawn only from `vcc status --json` output; see FIGURE_MANIFEST.md. | `arc_submission_v1_expectations.md` | pending (external calibration) | | | | ✓ |
