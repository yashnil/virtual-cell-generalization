"""Pathway residual model v1 — nested-LOCO, low-capacity, per-context evaluation.

Outer folds are the immutable four LOCO splits. Model family, ridge penalty and
correction shrinkage are chosen by inner pseudo-LOCO over the three source
contexts only; the outer target is touched exactly once, for evaluation.

Controls: matched-random pathway representations (full nested selection re-run on
each replicate, so the null is not handicapped) and Reactome as an independent
ontology.

No deep model. No Arc predictions. No gene-level lift-back.
...
def run_all_folds(
    ...
    *,
    ...
    for fold in folds:
        ...
        sel = pr.select_by_inner_loco(Y, src, families=families, **kw)
        out = pr.fit_and_predict_outer(Y, t, src, sel, **kw)
        B, R_hat = out["baseline"], out["residual_hat"]
        obs = Y[t]
        ...
        for fam in families:
            ...
            o = pr.fit_and_predict_outer(Y, t, src, pr.Selection(fam, a, lam), **kw)
            per_family[fam] = {
                "alpha": a,
                "lam": lam,
